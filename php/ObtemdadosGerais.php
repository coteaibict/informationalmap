<?php
	...
	if($_GET["tipo"] == "M"){
		...
		$res = $con->query("SELECT  e.cod_estado, e.uf, sum(p.Populacao) as 'População', sum(p.PIB) as 'Pib' , sum(p.PIB)/sum(p.Populacao) as 'Pib percapita', 
							...
							sum((select dc.valor from dadoscenso2010 dc where dc.cod_municipio = m.cod_municipio and informacao like  'Homens' limit 1)*(select dc.valor from dadoscenso2010 dc where dc.cod_municipio = m.cod_municipio and informacao like  'Rendimento Médio Mensal NULLHomens' limit 1))/sum((select dc.valor from dadoscenso2010 dc where dc.cod_municipio = m.cod_municipio and informacao like  'Homens' limit 1)) as 'Média rendimento homens',
							sum((select dc.valor from dadoscenso2010 dc where dc.cod_municipio = m.cod_municipio and informacao like  'Mulheres' limit 1)*(select dc.valor from dadoscenso2010 dc where dc.cod_municipio = m.cod_municipio and informacao like  'Rendimento Médio Mensal NULLMulheres' limit 1))/sum((select dc.valor from dadoscenso2010 dc where dc.cod_municipio = m.cod_municipio and informacao like  'Mulheres' limit 1)) as 'Média rendimento mulheres',
